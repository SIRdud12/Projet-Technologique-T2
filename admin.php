...
function afficherProjetsEnCours($conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo '<form method="post" action="">';
            echo '<div style="border: 1px solid #000; padding: 10px; margin-bottom: 10px;">';
            echo '<p>ID: ' . $row["IDProjet"] . '<br>Nom: ' . $row["nomProjet"] . '<br>Description: ' . $row["descriptionProjet"] . '<br>Durée: ' . $row["Duree_projet"] .  '<br>Statut: ' . $row["Statu"] . '<br>Budget: ' . $row["budjet"] . '</p>';
            echo '<input type="hidden" name="projet_id" value="' . $row["IDProjet"] . '">';
            echo '<a href="projet.php?projet_id=' . $row["IDProjet"] . '">Gérer</a>';
...
if (isset($_POST['creer_projet'])) {
    ...
    $sql = "INSERT INTO Projet (nomProjet, Duree_projet, descriptionProjet,  Statu, budjet) VALUES ('$nom_projet', '$duree_projet', '$description',  '$statu', '$budjet')";
